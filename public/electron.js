...
function ensureDir() {
  try { if (!fs.existsSync(LEIF_DIR)) fs.mkdirSync(LEIF_DIR, { recursive: true }); }
  catch (e) { console.error("Could not create data folder:", e); }
}
...
function writeJson(file, data) {
  try { ensureDir(); fs.writeFileSync(file, JSON.stringify(data, null, 2)); return true; }
  catch (e) { console.error("Write error", file, e); return false; }
}

function createWindow() {
  const win = new BrowserWindow({
    width: 1080, height: 740, minWidth: 720, minHeight: 480,
    backgroundColor: "#0c120f", titleBarStyle: "hiddenInset",
    webPreferences: {
...
ipcMain.handle("open-data-folder", () => { ensureDir(); shell.openPath(LEIF_DIR); return LEIF_DIR; });
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1500,
        system: "You are Leif, a friendly assistant living inside the user's personal note-taking app. You can see the user's notes below and should use them to answer questions, find information, draft responses, or summarize. If the notes don't cover something, say so plainly.\n\n=== USER'S NOTES ===\n" + (notesContext || "(no notes yet)"),
        messages,
      }),
    });
...
app.whenReady().then(() => { ensureDir(); createWindow(); });
